...
var CreatorsCloudStorageClient = class _CreatorsCloudStorageClient {
  ...
  uploadFile(_0, _1, _2) {
    return __async(this, arguments, function* (bucketName, fileName, fileContent, options = {}) {
      var _a;
      try {
        const params = __spreadValues({
          Bucket: bucketName,
          Key: fileName,
          Body: options.compress ? yield gzip(fileContent) : fileContent
        }, options.compress && { ContentEncoding: "gzip" });
        const command = new import_client_s3.PutObjectCommand(params);
        if (options.compress) {
          console.debug(`Compression percentage: ${(((_a = params.Body) == null ? void 0 : _a.length) / fileContent.length * 100).toFixed(2)}% of original size`);
        }
  ...
  downloadJson(bucketName, fileName, schema) {
    return __async(this, null, function* () {
      ...
      if (downloadResult.ContentEncoding === "gzip") {
        const fileBuffer = Buffer.from(yield downloadResult.Body.transformToByteArray());
        console.log(`Decompressing file ${fileName} from bucket ${bucketName}`);
        const unzippedResult = yield new Promise((resolve, reject) => {
          const gunzip = (0, import_zlib.createGunzip)();
        ...
        jsonString = yield downloadResult.Body.transformToString();
      }
